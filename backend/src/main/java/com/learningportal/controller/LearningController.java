...
public class LearningController {
    ...
    @Operation(summary = "Search questions", description = "Search and filter practice questions")
    public ResponseEntity<Page<PracticeQuestionDTO>> searchQuestions(
            @Parameter(description = "Search term") @RequestParam(required = false) String q,
            @Parameter(description = "Difficulty level") @RequestParam(required = false) DifficultyLevel difficulty,
            @Parameter(description = "Question type") @RequestParam(required = false) QuestionType type,
            @Parameter(description = "Page number") @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "Page size") @RequestParam(defaultValue = "20") int size) {

        Pageable pageable = PageRequest.of(page, size);
        Page<PracticeQuestion> questions = learningService.searchQuestions(type, difficulty, q, pageable);
        Page<PracticeQuestionDTO> dtos = questions.map(this::convertToPracticeQuestionDTO);
        return ResponseEntity.ok(dtos);
    }
