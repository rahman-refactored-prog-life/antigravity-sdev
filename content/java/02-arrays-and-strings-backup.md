...
**Prerequisites**:
- [Variables] → You need to store *one* thing before *many*.
- [Memory] → Understanding Stack vs Heap is crucial.

**This Concept**: **Arrays & Strings** ← You are here

**Next Steps**:
- [HashMaps] → Uses arrays to store key-value pairs.
- [Dynamic Programming] → Uses arrays (tables) to cache results.
...
graph TD
    ...
    B --> F[Heaps / Priority Queues]
```

### Mental Model & Analogies

**Analogy 1 (Physical)**: **The Egg Carton**
An egg carton has 12 fixed slots. 
- **Access**: You can pick the 5th egg instantly.
- **Fixed Size**: You can't squeeze a 13th egg in without buying a bigger carton.

**Analogy 2 (Digital)**: **Spreadsheet Rows**
Row 1 to Row 100. You can click Row 50 instantly. You cannot "insert" a row between 49 and 50 without shifting 50-100 down (expensive!).

**Analogy 3 (String Specific)**: **The Tattoo**
A String in Java/Python is like a tattoo. You can't "edit" a typo. You have to cover it up (garbage collect) and get a whole new tattoo.

### Common Misconceptions (Avoid These!)

❌ **Misconception 1**: "Arrays and Lists are the same."
✅ **Reality**: An Array is a fixed block of memory. A List (like Python's `list`) is a dynamic wrapper *around* an array that handles resizing for you.
💡 **Why**: Languages hide the resizing logic. In C/Java, the difference is strict.
...
💡 **Why**: Security and Thread Safety (explained in Layer 7).

### Industry Use Cases

**How FAANG Uses This**:
- **Google BigTable**: Stores petabytes of data as byte arrays.
- **Kafka**: Logs are immutable append-only arrays of messages.
- **Redis**: Uses "Simple Dynamic Strings" (SDS) to allow O(1) length checks and buffer safety.
...
#### Formal Definition
An **Linear Array** is a collection of finite number of $n$ homogeneous data elements such that:
...
1.  **Dynamic Array (`ArrayList`/`vector`)**: Auto-resizes by creating a new array 2x size and copying everything over.
2.  **StringBuilder**: A mutable "String" that is actually a Char Array under the hood. Fills up, then expands.
3.  **Circular Buffer**: Connects the end back to the start. Used in Queues/Kafka.

### Level 5: Interview (Master Level)
**Canonical Patterns**:
1.  **Iterate**: Simple `for` loop.
2.  **Two Pointer**: Meeting in the middle (Palindrome) or slow/fast (Cycle Detection).
3.  **Sliding Window**: Contiguous subarrays.
4.  **Prefix Sum**: Pre-calculating sums for O(1) range queries.
...
| Operation | Array | String (Immutable) | StringBuilder |
| :--- | :--- | :--- | :--- |
| **Access** | O(1) | O(1) | O(1) |
| **Search** | O(N) | O(N) | O(N) |
| **Insert/Delete** | O(N) | O(N) (Copy) | O(N) |
| **Concatenation** | - | O(N^2) (in loop) | O(N) |

### Key Methods (Java)
- **Array**: `arr.length`, `Arrays.sort(arr)`, `Arrays.copyOf(arr, len)`
- **String**: `s.length()`, `s.charAt(i)`, `s.substring(start, end)`, `s.toCharArray()`
- **StringBuilder**: `sb.append()`, `sb.reverse()`, `sb.toString()`

---

## 10. References & Further Reading

1.  **Visualizations**: [VisualAlgo - Sorting/Arrays](https://visualalgo.net/en/sorting)
2.  **Documentation**: [Oracle Java Arrays](https://docs.oracle.com/javase/tutorial/java/nutsandbolts/arrays.html)
3.  **Deep Dive**: "Effective Java" Items on Immutability.
